...
# skip all of the simulations above and just use stored data
data(storeFCVA)
data(storeFCSD)
data(FCVA)
data(FCSD)
...
alldat = NULL
for(i in 1:length(FCVA[,1])) alldat = rbind(alldat,
  data.frame(type = 'FCVA', dist = FCVA[i,'meanDist'], 
    svgm = storeFCVA[i,]),
  data.frame(type = 'FCSD', dist = FCVA[i,'meanDist'], 
    svgm = storeFCSD[i,]))
...
# create plot
pdf('FCVAvsFCSD4Fig.pdf', height = 8, width = 14)
  ...
  points(1:11,apply(storeFCVA,1,mean), pch = 19)
  points((1:11)+.3,apply(storeFCSD,1,mean), pch = 19)
  legend(.2,-2.5, legend = c('FCVA','FCSD'), pch = c(22,15), 
	  col = c('black','grey70'), cex = 2)
  axis(side=1, (1:11)+.15, labels=as.character(round(FCVA[1:11,'meanDist'],1)), 
    cex.axis = 1.5)
